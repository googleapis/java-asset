...
function completenessCheck() {
  ...
  mvn dependency:list -f pom.xml -DexcludeArtifactIds=commons-codec,commons-loggin,annotations,animal-sniffer-annotations,protobuf-java-util -DincludeScope=runtime -Dsort=true | grep '\[INFO]    .*:.*:.*:.*:.*' | sed -e 's/ --.*//' >.org-list.txt
